...
export const AddressingNotesTable = ({
  notes, page, rowsPerPage, onPageChange, onRowsPerPageChange, onAddressNote,
}) => {
    ...
    <>
      <Paper elevation={0} sx={{ border: "1px solid", borderColor: "divider", overflow: "auto" }}>
        <Table sx={{ minWidth: 1050 }}>
          <TableHead><TableRow>
            {["Nota/Ordem", "Fornecedor", "SKUs", "Recebido", "Endereçado", "Pendente", "Endereçamento", "Recebimento", "Data", "Ações"].map((label) => (
              <TableCell key={label} sx={{ fontWeight: 700 }}>{label}</TableCell>
            ))}
          </TableRow></TableHead>
          <TableBody>
            {visibleNotes.length === 0 ? (
              <TableRow><TableCell colSpan={10}><Box sx={{ py: 6, textAlign: "center", color: "text.secondary" }}>Nenhuma nota encontrada.</Box></TableCell></TableRow>
              ...
              <TableRow key={note.id} hover>
                ...
                <TableCell><Button variant="contained" size="small" onClick={() => { setOpenedNote(note); setNoteOpen(true); }}>Abrir nota</Button></TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
        <TablePagination component="div" count={notes.length} page={safePage} rowsPerPage={rowsPerPage}
      ...
      >
        ...
        <DialogContent>
          {openedNote && <Stack spacing={2} sx={{ mt: 1 }}>
            ...
            <Paper variant="outlined" sx={{ overflow: "auto" }}>
              <Table size="small">
                ...
                <TableBody>{openedNote.items.map((item) => (
                  <TableRow key={item.id}>
                    <TableCell>{item.sku}</TableCell><TableCell>{item.ean}</TableCell>
                    <TableCell>{item.productName}</TableCell><TableCell>{item.receivedQty}</TableCell>
                    <TableCell>{item.addressedQty}</TableCell><TableCell>{item.pendingQty}</TableCell>
        ...
        <DialogActions>
          ...
          <Button variant="contained" disabled={pendingIds.length === 0}
